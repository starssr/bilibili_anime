...
class bilibiliApiRequest
{
	...
	// Add New , Range browser head.
    {
      $arr = array(
        ...
        $url = "http://api.bilibili.com/x/space/bangumi/follow/list?type=1&follow_status=0&pn=1&ps=15&vmid=$uid";
        $referer = "Referer: http://www.bilibili.com";
        $ch = curl_init();
    ...
    public function __construct($uid,$cookie)
    {
        ...
        {
            $url = "http://api.bilibili.com/x/space/bangumi/follow/list?type=1&follow_status=0&pn=$i&ps=15&vmid=$uid";
            $cookie = "";
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
